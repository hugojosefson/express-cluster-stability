...
import queueFactory from './queue-factory.mjs'
...
export default (masterFunction, options) => {
  ...
  const messageHandler = worker => message => {
    ...
    if (message === 'crashing') {
      log.warn(
        `Worker ${worker.id} with PID ${worker.process.pid} is crashing. Disconnecting it...`
      )
      crashingWorkers.set(
        worker.id,
        setTimeout(() => {
          log.warn(
            `Worker ${worker.id} with PID ${worker.process.pid} hasn't disconnected after ${workerKillTimeout} ms. Killing it...`
          )
          worker.kill()
        }, workerKillTimeout)
      )
  ...
  cluster.on('exit', (worker, code, signal) => {
    log(
      `Worker ${worker.id} with PID ${worker.process.pid} exited with code ${code} because of signal ${signal}`
    )
